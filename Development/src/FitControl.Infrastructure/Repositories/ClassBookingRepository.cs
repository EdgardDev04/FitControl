...
namespace FitControl.Infrastructure.Repositories
{
    internal class ClassBookingRepository : IClassBookingRepository
    {
        ...
        public async Task GetByDateAsync(DateTime date) => await _context.ClassBookings.FirstOrDefaultAsync(cb => cb.BookingDate == date);

        public async Task<ClassBooking?> GetByIdAsync(int id) => await _context.ClassBookings.FindAsync(id);

        public async Task<IEnumerable<ClassBooking>> GetAllByMemberIdAsync(int memberId) => await _context.ClassBookings.Where(cb => cb.MemberId == memberId).ToListAsync();

        public async Task<IEnumerable<ClassBooking>>GetAllByStatusAsync(BookingStatus status) => await _context.ClassBookings.Where(cb => cb.Status == status).ToListAsync();
