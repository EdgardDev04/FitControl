...
namespace FitControl.Infrastructure.Persistence.Repositories
{
    internal class MembershipRepository : IMembershipRepository
    {
        ...
        public async Task<ICollection<Membership>> GetAllByDateAsync(DateTime startdate, DateTime enddate) => await _context.Memberships.Where(m => m.StartDate >= startdate && m.EndDate <= enddate).ToListAsync();
        public async Task<ICollection<Membership>> GetAllByStatusAsync(MembershipStatus status) => await _context.Memberships.Where(m => m.Status == status).ToListAsync();
        public async Task<Membership?> GetByIdAsync(int id) => await _context.Memberships.FindAsync(id);
        public async Task<ICollection<Membership>> GetByMemberIdAsync(int memberId) => await _context.Memberships.Where(m => m.MemberId == memberId).ToListAsync();
        public async Task<ICollection<Membership>> GetByMembershipPlanIdAsync(int membershipPlanId) => await _context.Memberships.Where(m => m.MembershipPlanId == membershipPlanId).ToListAsync();
